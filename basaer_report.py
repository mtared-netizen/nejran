...
def page_mutun_milestones(c, pg):
    ...
    txt(c, "مسابقة الأمن الفكري الثانية 1439هـ — 3,405 مشاركاً من 30+ مدينة سعودية ودول عربية",
        W-MARGIN-5, cy-4*mm, "Cairo-Bold", 9, GOLD, "right")
    txt(c, "2,129 مشاركاً ذكوراً  |  1,276 مشاركةً  |  شملت: الرياض، جدة، الدمام، الطائف، أبها، تبوك، والمغرب، اليمن، مصر، باكستان",
...
def page_mafateeh_sharurah(c, pg):
    ...
    txt(c, "يُنفَّذ بالتعاون مع أمارة منطقة نجران | يستهدف الأئمة والمساجد والمتعلمين في محافظة شرورة",
    ...
    # ── 1444هـ stats ──
    txt(c, "مؤشرات مشروع مفاتيح الخير شرورة — 1444هـ (الأشمل)", W-MARGIN-4, H-64*mm,
        "Cairo-Bold", 9, NAVY, "right")
    ...
    rect(c, MARGIN, H-106*mm, CW, 10*mm, fill=DKGREEN)
    txt(c, "إجمالي المستفيدين في شرورة 1444هـ: أكثر من 33,000 مستفيد ومستفيدة",
        W-MARGIN-5, H-99*mm, "Cairo-Bold", 9.5, WHITE, "right")
    txt(c, "العائد الاقتصادي للتطوع: 35,200 ريال  |  عدد المتطوعين: 26 متطوعاً",
        W-MARGIN-5, H-104*mm, "Cairo-Regular", 8, GOLD, "right")
    ...
    analysis_box(c, MARGIN, H-109*mm, CW, analysis_txt, "التحليل التشخيصي")
...
def page_programs_objectives(c, pg):
    ...
    findings = [
        ...
        (RED,
         ...
         "غائب عملياً عن معظم البرامج — المسابقة الإلكترونية ومنصة متون الاستثنائان الوحيدان، "
         "بينما تبقى سائر الأنشطة ميدانية تقليدية خالية من أي توظيف تقني منهجي. "
         "لا توجد استراتيجية رقمية مكتوبة رغم أن تجربة 2024 أثبتت إمكانية الوصول لمليون مشاهد."),
    ]
    cy2 = ty - 10*mm
    for col_f, title_f, desc_f in findings:
        txt(c, title_f, W-MARGIN-4, cy2, "Cairo-SemiBold", 8.5, GOLD, "right")
        txt(c, desc_f,  W-MARGIN-4, cy2-5*mm, "Cairo-Regular", 8, col_f, "right")
        cy2 -= 12*mm
