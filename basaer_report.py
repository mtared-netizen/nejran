...
def page_org_overview(c, pg):
    ...
    # المادة الخامسة
    y5 = H-62*mm
    rect(c, MARGIN, y5-4*mm, CW, 6*mm, fill=NAVY)
    txt(c, "المادة الخامسة — أهداف الجمعية الأساسية (اللائحة المعتمدة)",
        W-MARGIN-4, y5-0.5*mm, "Cairo-Bold", 10, GOLD, "right")

    objectives = [
        ("الهدف الأول",
         "الدعوة إلى الله وتوعية الجاليات غير المسلمة في منطقة نجران وتقديم مستلزماتها."),
        ("الهدف الثاني",
         "الإرشاد الديني وتوعية المسلمين من مواطنين ومقيمين بثوابت الدين الإسلامي "
         "ومبادئه وفق منهج أهل السنة والجماعة بالوسطية والاعتدال."),
        ("الهدف الثالث",
         "تعليم المسلمين الجدد أحكام الإسلام وتأهيلهم وتقديم المتطلبات اللازمة لهم."),
        ("الهدف الرابع",
         "استخدام التقنية الحديثة بجميع أنواعها في نشر العلم الشرعي، والدعوة إلى الله "
         "في جميع مجالاتها، وتحقيق جميع أهداف الجمعية."),
    ]
    cy = y5 - 8*mm
...
def page_dawa(c, pg):
    ...
    analysis = ("تكشف بيانات البرامج الدعوية عن مسار متذبذب وغير مستقر خلال الفترة 2019-2025. "
                ...
                "مقابل توسع البرامج العلمية الأكاديمية كمتون. ويُسجَّل عام 2023 استثناءً واضحاً "
...
def page_governance(c, pg):
    ...
    for name, pct, col, note in criteria:
        ...
        txt(c, f"{pct}%", MARGIN+28*mm+bw+2*mm, cy-3.5*mm, "Cairo-Bold", 8, col, "left")
...
def page_swot(c, pg):
    ...
    txt(c, "مصفوفة SWOT الموسعة — 44 نقطة", W-MARGIN, H-19*mm, "Cairo-Black", 15, NAVY, "right")
    line(c, MARGIN, H-22*mm, W-MARGIN, H-22*mm, GOLD, 1)
    ...
    quadrants = [
        ...
        (RED, "نقاط الضعف (Weaknesses) — 12 نقطة", [
...
def page_gaps(c, pg):
    ...
    gaps = [
        (RED,     "01", "فجوة الهوية الاستراتيجية — الأخطر والأعمق",
         ...
         "هو انزياح تدريجي غير مخطط تراكم على مدى 6 سنوات. قسم الجاليات شبه فارغ من الكوادر "
         "رغم أنه المحور الأول في الاسم الرسمي. انعدام المترجمين يجعل الوصول للجاليات "
         "غير الناطقة بالعربية شبه مستحيل."),
...
def page_programs_objectives(c, pg):
    ...
    # Alignment matrix
    programs = [
        # (program, obj1, obj2, obj3, obj4, note)
        # obj4 = استخدام التقنية الحديثة في نشر العلم والدعوة وتحقيق الأهداف
        ("برنامج متون (تحفيظ المتون العلمية)",
         False, True, False, True,
         "يخدم الهدف الثاني (الإرشاد الديني) والرابع (منصات رقمية في نشر العلم)"),
        ("مشروع تفطير الصائمين",
         False, False, False, False,
         "عمل خيري ميداني — لا يندرج تحت التقنية ولا الدعوة المباشرة للجاليات"),
        ("الكلمات والمحاضرات الدعوية",
         True, True, False, False,
         "يخدم الهدفين الأول والثاني — دعوة الجاليات وإرشاد المسلمين"),
        ("المسابقة الرمضانية الإلكترونية",
         False, True, False, True,
         "منصة رقمية تجسّد الهدف الرابع (التقنية) وتخدم الهدف الثاني (الإرشاد)"),
        ("برامج الجاليات الميدانية",
         True, False, True, False,
         "مباشر للهدف الأول والثالث — لا يستخدم التقنية بشكل محوري"),
        ("المجالس والدروس العلمية",
         False, True, False, False,
         "إرشاد ديني للمسلمين — لا يستهدف الجاليات ولا يوظّف التقنية رسمياً"),
        ("توزيع المصاحف والمواد الدعوية",
         True, False, True, False,
         "يخدم الهدفين الأول والثالث — مواد مطبوعة ميدانية لا رقمية"),
        ("برنامج يوم في الحرم",
         False, False, True, False,
         "تأهيل المسلمين الجدد — برنامج ميداني لا يرتكز على التقنية"),
        ("مشروع المصليات المتنقلة",
         False, True, False, False,
         "إرشاد ديني للمرابطين في الحد الجنوبي — برنامج لوجستي ميداني"),
        ("السلال الغذائية والتمور",
         False, False, False, False,
         "عمل خيري بحت — لا يرتبط بأهداف الدعوة والتقنية"),
    ]
    ...
    # Alignment summary
    ty -= 4*mm
    rect(c, MARGIN, ty-28*mm, CW, 28*mm, fill=NAVY)
    txt(c, "خلاصة تحليل مدى توافق البرامج مع الأهداف", W-MARGIN-4, ty-4*mm,
        "Cairo-Bold", 10, GOLD, "right")
    findings = [
        ("الهدف الأول — الدعوة للجاليات:",
         "يُجسَّد في برامج الجاليات الميدانية والكلمات الدعوية فقط — الأقل حضوراً في موازنة البرامج "
         "رغم أنه المحور الأول في الاسم الرسمي والغرض التأسيسي للجمعية."),
        ("الهدف الثاني — الإرشاد للمسلمين:",
         "يُمثّل الغالبية العظمى من أنشطة الجمعية (متون، دروس، محاضرات، مسابقة) — "
         "تضخّم استراتيجي غير مُعلن على حساب هدف الجاليات التأسيسي."),
        ("الهدف الثالث — تعليم المسلمين الجدد:",
         "ضعيف الحضور رغم 931 مسلماً منذ التأسيس — برامج تأهيل المُسلَمين لا ترقى "
         "لحجم الاحتياج الفعلي وتفتقر للمنهجية والاستمرارية."),
        ("الهدف الرابع — استخدام التقنية:",
         "يُجسَّده برنامج متون (منصات رقمية) والمسابقة الإلكترونية فحسب — "
         "معظم البرامج ميدانية تقليدية والتقنية لا تُوظَّف بشكل منهجي لتضخيم الأثر الدعوي."),
    ]
    cy2 = ty - 10*mm
...
def page_conclusion(c, pg):
    ...
    conclusion2 = ("لكنها في الوقت ذاته تسمح لأكثر مؤشراتها جوهرية — إسلام الجاليات — أن ينهار "
                   "85% دون أن تتخذ قراراً استراتيجياً واضحاً لمعالجته. التحدي الأكبر ليس "
    ...
    priorities = [
        (RED,     "01", "القرار الهوياتي",
         "الجمعية بحاجة لإجابة صريحة ومُعلنة: هل هي مؤسسة دعوة للجاليات أم مرجعية علمية للمجتمع؟"),
        (AMBER,   "02", "إنقاذ مؤشر المسلمين الجدد",
         "تراجع من 72 إلى 11 في ست سنوات ليس قدراً — هو نتيجة غياب منظومة مؤسسية."),
...
section_div(c, "05", "الرمضانيات والتطوع", "التفطير والمسابقة والمتطوعون", pg); pg+=1
